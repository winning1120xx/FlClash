...
class ClashService with ClashInterface {
  ...
  startServer({
    ...
    int? port;
    helper = await system.checkIsAdmin();
    if (Platform.isWindows && helper) {
      port = await request.startCoreByHelper();
    }
    port ??= await _startCore();
    _connectCore(port);
  }

  Future<int> _startCore() async {
    process = await Process.start(
      appPath.corePath,
      [],
    );
    final completer = Completer<int>();
    final timer = Timer(Duration(milliseconds: 3000), () {
    ...
    process?.stdout
      ...
      if (output.startsWith("[port]: ")) {
        ...
        completer.complete(int.tryParse(portString) ?? 0);
        timer.cancel();
      }
    });
